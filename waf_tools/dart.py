...
def check_dart(conf):
	...
	try:
		...
		res = res and conf.find_file('dart/io/io.hpp', includes_check)
		res = res and conf.find_file('dart/io/urdf/urdf.hpp', includes_check)
		conf.end_msg('ok')
		try:
		...
		res = res and conf.find_file('libdart-io.so', libs_check)
		res = res and conf.find_file('libdart-io-urdf.so', libs_check)
		conf.end_msg('ok')
		conf.env.INCLUDES_DART = includes_check + more_includes
		conf.env.LIBPATH_DART = libs_check
		conf.env.LIB_DART = ['dart', 'dart-io', 'dart-io-urdf']
