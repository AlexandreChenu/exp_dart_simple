...
#ifndef NEURAL_MAPE_GEN_MLP_HPP
#define NEURAL_MAPE_GEN_MLP_HPP
...
namespace sferes {
    namespace gen {
        ...
        public:
            ...
            GenMlp():_hidden_neurons{} {}
            GenMlp& operator=(const GenMlp& o) {
                static_cast<nn::NN<N, C>& >(*this)
                        = static_cast<const nn::NN<N, C>& >(o);
                return *this;
            }
            GenMlp(const GenMlp& o) {
                *this = o;
            }
            void init() {
		...
		if (Params::mlp::layer_2_size > 0) {
		    size_t layer_2_size = Params::mlp::layer_2_size;
                    nbs_hidden.push_back(layer_2_size);
		}
            ...
            void _random(size_t nb_inputs,
                 const std::vector<size_t>& nbs_hidden,
                 size_t nb_outputs) {
...
//                std::cout << std::endl;
                ...
                for (size_t j = 0; j < nbs_hidden.size(); ++j) {
                    ...
                    for (size_t i = 0; i < nbs_hidden[j]; ++i)
                        _hidden_neurons[j].
                                push_back(this->add_neuron(
                                std::string("layer") + std::to_string(j) + std::string("_h") +
                                std::to_string(i)));
                }
                // connections
...
//                    for (size_t i = 0; i < nbs_hidden[j]; ++i) {
//                        w.random();
//                        this->add_connection(this->get_input(nb_inputs), this->_hidden_neurons[j][i],w);
//                    }
//                }
//                // bias outputs too
...
//                }
            ...
            void full_connect_random(const std::vector<vertex_desc_t> v1,
            ...
            void cross(const GenMlp& o, GenMlp& c1, GenMlp& c2) {
                if (misc::flip_coin()) {
                    c1 = *this;
                    c2 = o;
                } else {
